 ...
 * @date 2021-10-19
 */
class Yearbra_model extends MY_Model
{
	...
	{
		$yearpr_id = checkEncryptData($data['yearpr_id']);
		$this->set_table_name($this->my_table);
		$this->set_where("$this->my_table.yearpr_id = $yearpr_id");
	...
	{
		$this->set_table_name($this->my_table);
		$this->set_where("$this->my_table.yearpr_id = $id");
	...
	public function create($post)
	{
		...
		$data = array(
			'yearpay' => $post['yearpay'], 'monthpay' => $post['monthpay'], 'rf_name_id' => $post['rf_name_id'], 'month_mony_sso' => str_replace(",", "", $post['month_mony_sso']), 'month_de_ssop' => str_replace(",", "", $post['month_de_ssop']), 'month_de_ssoc' => str_replace(",", "", $post['month_de_ssoc'])
		);
		$this->set_table_name($this->my_table);
	...
	public function read($start_row = FALSE, $per_page = FALSE)
	{
		...
		if ($search_field != '' && $value != '') {
			...
			if ($search_field == 'rf_branch_id') {
				$search_method_field = "tb_person.rf_branch_id";
				$search_method_value = "= '$value'";
			}

			$where	.= ($where != '' ? ' AND ' : '') . " $search_method_field $search_method_value AND yearpay ='$payyear'";
		...
		if ($where != '') {
			...
			$this->db->join('tb_company', 'tb_person.rf_company =  tb_company.company_id', 'left');
		...
		$this->db->select("$this->my_table.*
				, tb_person.name_id AS rfNameID
				, tb_person.emp_barcode AS rfNameIDEmpBarcode
				...
				, tb_person.end_date AS rfNameIDEndDate 
				, tb_person.ageb_start AS rfNameIDAgebStart
				, tb_pername.pre_name AS rfPernameIdPreName				
				, tb_branch.branch_nick AS rfBranchIdBranchNick
				, tb_person.person_mumsso AS rfPersonIDPersonMumsso
				, yearpr_totalmony AS totalMonyPersonYear
				, yearpr_totaltax AS totalTaxPersonYear
				, yearpr_totalsso AS totalSsoPersonYear
	...
	public function sumexcel($start_row = FALSE, $per_page = FALSE)
	{
		...
		if ($search_field != '' && $value != '') {
			...
			if ($search_field == 'rf_branch_id') {
				$search_method_field = "tb_person_2.rf_branch_id";
				$search_method_value = "= '$value'";
			}
	...
	{
		$data = array(
			'yearpay' => $post['yearpay'], 'monthpay' => $post['monthpay'], 'rf_name_id' => $post['rf_name_id'], 'month_mony_sso' => str_replace(",", "", $post['month_mony_sso']), 'month_de_ssop' => str_replace(",", "", $post['month_de_ssop']), 'month_de_ssoc' => str_replace(",", "", $post['month_de_ssoc']), 'month_ckrun' => $post['month_ckrun']
		);

		if (!empty($data)) {
			$yearpr_id = checkEncryptData($post['encrypt_yearpr_id']);
			$this->set_table_name($this->my_table);
			$this->set_where("$this->my_table.yearpr_id = $yearpr_id");
