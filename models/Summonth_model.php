 ...
 * @date 2021-10-19
 */
class Summonth_model extends MY_Model
{
	...
	*/
	public function read($start_row = FALSE, $per_page = FALSE)
	{
		...
		if($search_field != '' && $value != ''){
			...
			if($search_field == 'monthpay'){
				$search_method_field = "CONCAT_WS(' ', tb_paymonth_1.paymonth_id, tb_paymonth_1.paymonth)";
				$search_method_value = "LIKE '$value%'";				
			}
			if($search_field == 'rf_name_id'){
				$search_method_field = "CONCAT_WS(' ', tb_person_2.emp_name, tb_person_2.emp_surname)";
				$search_method_value = "LIKE '%$value%'";				
			}
		...
		$this->db->select("$this->my_table.*, tb_paymonth_1.paymonth_id AS monthpayPaymonthId, tb_paymonth_1.paymonth AS monthpayPaymonth
				, tb_person_2.emp_name AS rfNameIdEmpName, tb_person_2.emp_surname AS rfNameIdEmpSurname
				");
		$this->db->join('tb_paymonth AS tb_paymonth_1', "$this->my_table.monthpay = tb_paymonth_1.paymonth_id", 'left');
		$this->db->join('tb_person AS tb_person_2', "$this->my_table.rf_name_id = tb_person_2.name_id", 'left');
