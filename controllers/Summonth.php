...
class Summonth extends CRUD_Controller
{
	...
	public function update()
	{
		...
		if ($message != '') {
			...
			if($result == false){
				...
				$message = '<strong>บันทึกข้อมูลเรียบร้อย</strong>' . $this->Summonth->error_message;
				$ok = TRUE;
			}
			$json = json_encode(array(
						'is_successful' => $ok,
	...
	public function export_excel() 
	{
		...
		$objPHPExcel->getActiveSheet()->SetCellValue('B1', 'ปี');
		$objPHPExcel->getActiveSheet()->SetCellValue('C1', 'เดือน');
		$objPHPExcel->getActiveSheet()->SetCellValue('D1', 'ชื่อพนักงาน');
		$objPHPExcel->getActiveSheet()->SetCellValue('E1', 'ยอดเงินคิดประกันสังคมประจำเดือน');
		$objPHPExcel->getActiveSheet()->SetCellValue('F1', 'ยอดเงินหักประกันสังคมประจำเดือนส่วนพนง');
		$objPHPExcel->getActiveSheet()->SetCellValue('G1', 'ยอดเงินหักประกันสังคมประจำเดือนส่วนนายจ้าง');
		$objPHPExcel->getActiveSheet()->SetCellValue('H1', 'สถานะปิดเดือน');
